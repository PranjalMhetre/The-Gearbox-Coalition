  ...
  return (
    <>
      ...
      >
        ...
        <h1 className='text-2xl font-bold'>{`Has Field: ${team.fields.hasField ? 'yes' : 'no'}`}</h1> 
        <h1 className='text-2xl'>{`Has Business: ${team.fields.hasBusinessSide ? 'yes' : 'no'}`}</h1>
        <h1 className='text-2xl text-center'>--- Comp Record ---</h1>
        <p className='text-xl'>{documentToReactComponents(team.fields.compRecord)}</p>
      </Modal>

      <UnstyledButton onClick={() => setOpened(true)} className="bg-red-300 hover:bg-blue-300 transition-colors duration-300 w-[2rem] aspect-square rounded-[50%] inline-flex justify-center items-center text-white">
